...
export const checkPasswordIsEqual = (isPasswordIsEqual: boolean) => {
  if (!isPasswordIsEqual)
    throw new BadRequestException('Something went wrong', {
      cause: new Error(),
      description: 'Password not equal copy',
    });
};
...
export const checkTimeWell = (isTimeWell: boolean) => {
  if (!isTimeWell)
    throw new BadRequestException('Something went wrong', {
      cause: new Error(),
      description: 'Your request is too late',
    });
};
...
export const checkCorrectSecret = (isCorrectSecret: boolean) => {
  if (!isCorrectSecret)
    throw new BadRequestException('Something went wrong', {
      cause: new Error(),
      description: 'Your secret is incorrect',
    });
};
...
export const checkWaitList = (waitList: IWaitListLine[]): IWaitListLine[] => {
  return waitList.filter((item) => {
    return (
      (new Date().getTime() - item.answerDate) / 60000 <
      Number(process.env.TIME_DELAY_TO_TO_SEND_POST2)
    );
  });
};
